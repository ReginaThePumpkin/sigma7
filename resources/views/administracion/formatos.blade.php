...
@section('libreriasJS')
    ...
    <script src="{{asset('jQuery-TE_v.1.4.0/uncompressed/jquery-te-1.4.0.js')}}"  charset="utf-8" type="text/javascript"></script>
    <script src='{{asset('tinymce/tinymce.min.js')}}' type="text/javascript"></script>
    {{--<script src='{{asset('c3/c3.min.js')}}' type="text/javascript"></script>
    <script src='{{asset('c3/d3/d3.min.js')}}' type="text/javascript"></script>--}}
    <!-- Input Mask-->
    <script src="{{asset('jasny-bootstrap/js/jasny-bootstrap.min.js')}}" type="text/javascript"></script>
    <!-- Mis funciones -->
    <script src="{{asset('funciones/js/inicio.js')}}" type="text/javascript"></script>
    <script src="{{asset('funciones/js/caracteres.js')}}" type="text/javascript"></script>
    <script src="{{asset('funciones/js/calcula_edad.js')}}" type="text/javascript"></script>
    <script src="{{asset('funciones/js/stdlib.js')}}" type="text/javascript"></script>
    <script src="{{asset('funciones/js/bs-modal-fullscreen.js')}}" type="text/javascript"></script>
...
@section('scripts')
    <script>
        $(document).ready(function(e) {
            ...
            //Elimina las alertas de error de la datatable
            ...
            var oTableP = $('#dataTablePrincipal').DataTable({
                ...
                "sAjaxSource": "formatos/datatable-serverside/formatos",
        ...
        function nuevo_formato(){
            $("#myModal1").load("formatos/htmls/ficha_formato",function(response,status,xhr){ if(status == "success"){ tinymce.remove("#input");
                tinymce.init({
        ...
        function guarda_formato(){
            if(tinyMCE.get("input").getContent() != '' & $('#nombreF').val() != ''){
                ...
                var datosP = {_token:token ,formato:tinyMCE.get("input").getContent(), nombre:$('#nombreF').val()}
                $.post('formatos/files-serverside/addFormato',datosP).done(function( data ) {
        ...
        function actualiza_formato(){
            if(tinyMCE.get("input").getContent() != '' & $('#nombreF').val() != ''){
                var token = $('meta[name="csrf-token"]').attr('content');
                var datosP = {_token: token, formato:tinyMCE.get("input").getContent(), nombre:$('#nombreF').val(), id_f:$('#id_formatof').val()}
                $.post('formatos/files-serverside/updateFormato',datosP).done(function( data ) {
                    if (data==1){//si el paciente se Actualizó
                        $('#clickme').click(); $('#myModal1').modal('hide');
        ...
        function ficha_formato(nf, idf){
            $("#myModal1").load("formatos/htmls/ficha_formato",function(response,status,xhr){ if(status == "success"){
                $('#titulo_modal').text(nf); $('#btn_actualizar').removeClass('hidden'); $('#btn_guardar').remove(); $('#id_formatof').val(idf); $('#nombreF').val(nf);

                var token = $('meta[name="csrf-token"]').attr('content');
                var datosFts ={id_f:idf, _token:token}
                $.post('formatos/files-serverside/ficha_formato',datosFts).done(function(data1x){ tinymce.remove("#input");
                    tinymce.init({
                        selector:'#myModal1 #input',resize:false,height:$('#referencia').height()*0.48,theme: "modern",
                        plugins:
                            "table, charmap, emoticons, textcolor colorpicker, hr, image imagetools, image, insertdatetime, lists, noneditable, pagebreak, paste, preview, print, visualblocks, wordcount, code, importcss",
